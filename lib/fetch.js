...
export const searchArtistsByQuery = async (e) => {
	try {
		return await fetch(`${api_url}search/artists?query=` + e);
	} catch (e) {
		console.log(e);
	}
};

export const searchArtistsByQueryPaged = async (
	query,
	{ page = 1, limit = 20 } = {},
) => {
	try {
		...
		if (page != null) params.set("page", String(page));
		if (limit != null) params.set("limit", String(limit));
		return await fetch(`${api_url}search/artists?${params.toString()}`);
	} catch (e) {
		console.log(e);
	}
};
...
export const getArtistSongsByIdPaged = async (
	id,
	{ page = 1, limit = 30 } = {},
) => {
	try {
		...
		return await fetch(
			`${api_url}artists/${id}/songs${qs ? `?${qs}` : ""}`,
		);
	} catch (e) {
		console.log(e);
	}

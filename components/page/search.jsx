...
export default function Search({ className }) {
  ...
  const totalOptions = useMemo(() => {
    ...
    return (
      filteredRecent.length + suggestionItems.length + (hasSearchAction ? 1 : 0)
    );
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative z-10 w-full max-w-2xl mx-auto", className)}>
      <form
          ...
          <div className="p-2">
            ...
            {trimmed.length > 0 && !loading && (
              <button
                ...
                onMouseDown={(e) => e.preventDefault()}
                onMouseEnter={() =>
                  setActiveIndex(filteredRecent.length + suggestionItems.length)
                }
                onClick={() => runSearch(trimmed)}
                className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-accent ${
                  (
                    activeIndex ===
                    filteredRecent.length + suggestionItems.length
                  ) ?
                    "bg-accent"
                  : ""
                }`}>

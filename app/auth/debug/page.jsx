...
export default function AuthDebugPage() {
	...
	const runtime = useMemo(() => {
		...
		const redirectTo = `${computedSiteUrl}/auth/callback?next=${encodeURIComponent("/")}`;
		...
		let storedNext = null;
		try {
			storedNext = globalThis?.sessionStorage?.getItem(
				"mpplaygo.oauth.next",
			);
		} catch {
			storedNext = null;
	...
	return (
		<div className="min-h-screen px-6 py-10 max-w-3xl mx-auto">
			...
			<p className="text-muted-foreground mt-2">
				Use this page to capture the exact runtime URLs and cookie
				state.
			</p>
			...
			<div className="mt-6 space-y-3 rounded-xl border border-white/10 bg-secondary/10 p-4 text-sm">
				...
				<div>
					...
					<div className="font-mono break-words">
						{runtime.supabaseCookies.length ?
							runtime.supabaseCookies.join("\n")
						:	"-"}
					</div>
				</div>
			</div>

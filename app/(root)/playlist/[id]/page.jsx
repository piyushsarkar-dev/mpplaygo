...
export default function PlaylistPage({ params }) {
  ...
  if (loading) {
    return (
      <div className="container mx-auto py-6 md:py-10 px-4 md:px-6 max-w-7xl animate-pulse-slow">
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 mb-8 md:mb-12">
          <Skeleton className="mx-auto md:mx-0 w-48 h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 rounded-xl" />
        ...
        <div className="space-y-3">
          ...
          {Array.from({ length: 8 }).map((_, index) => (
            <div
              key={index}
              className="flex items-center gap-4 p-3 rounded-2xl bg-white/[0.03] border border-white/[0.05]">
  ...
  if (!playlist)
    return (
      <div className="p-10 text-center">Playlist not found or private</div>
    );
  ...
  const handleTogglePublic = async () => {
    ...
    try {
      ...
      setPlaylist(data);
      toast.success(
        nextValue ? "Playlist is now public" : "Playlist is now private",
      );
    } catch (e) {
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-8 mb-8 md:mb-12">
        ...
        <div className="flex-1 flex flex-col justify-center text-center md:text-left space-y-3 md:space-y-4">
          ...
          <div className="flex items-center justify-center md:justify-start gap-2 text-xs">
            <div
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${playlist.is_public ? "bg-green-500/10 text-green-500" : "bg-muted text-muted-foreground"}`}>
              {playlist.is_public ?
